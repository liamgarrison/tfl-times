...
module.exports = async function getDepartures (from, to, query = {}) {
  ...
  const times1LastJourney = times1Journeys && times1Journeys.length > 0 ? times1Journeys[times1Journeys.length - 1] : [];
  let times2Journeys = [];
  if (times1LastJourney.length > 0) {
    const times2 = await getTimes(getMinutesFromNow(times1LastJourney.startDateTime));
    times2Journeys = times2.data.journeys || [];
  }

  return chain([...times1Journeys, ...times2Journeys])
  .map(journey => {
    return {
